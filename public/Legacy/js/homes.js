$(function() {
	$(".draggable").draggable( {
		containment: "#containment-wrapper", cursor: 'move'
	}
	);
	$(document).click(function(evt) {
		console.log($(evt.target).hasClass('icon-edit')) if($(evt.target).hasClass('icon-edit') || $(evt.target).hasClass('selectSkin')) {}
		else {
...
$(document).on("mouseup", ".draggable", function() {
	var elem=$(this), pos=elem.position();
	elem.attr('data-top', pos.top) elem.attr('data-left', pos.left)
}
...
$("#save").click(function() {
	...
	$.ajax( {
		...
		, type: 'POST', url: '/home/' + username + '/save', contentType: 'application/json', data: widgets, success: function(result) {
...
function disp(divs) {
	...
	i++) {
		var thing= {
			"id": divs[i].getAttribute('id'), "y": divs[i].getAttribute('data-top'), "x": divs[i].getAttribute('data-left'), "z": divs[i].getAttribute('data-z'), "skin": divs[i].getAttribute('data-skin'), "data": divs[i].getAttribute('data-data')
		}
		a.push(thing);
	}
...
function showMenu(selector) {
	var elem=$(selector), id=elem.attr('data-id');
...
function changeSkin(selector, value) {
	var elem=$(selector), id=elem.attr('data-id');
	console.log(id) $('.edit-menu').removeClass('show');
	$('#' + id).fadeOut("slow", function() {
		$('#' + id).removeClass('widget_default_skin'), $('#' + id).removeClass('widget_bubble_skin'), $('#' + id).removeClass('widget_notepad_skin'), $('#' + id).removeClass('widget_metal_skin'), $('#' + id).removeClass('widget_golden_skin');
...
function deleteElement(selector) {
	var elem=$(selector), id=elem.attr('data-id'), type=elem.attr('data-type');
	...
	$.ajax( {
		...
		, type: 'POST', url: '/home/' + username + '/delete', contentType: 'application/json', data: value, success: function(result) {
			if (type !='w') {
...
jQuery.fn.center=function() {
	...
	this.css("top", Math.max(0, (($('.store_overlay').height() - $(this).outerHeight()) / 2) + $('.store_overlay').scrollTop()) + "px");
	this.css("left", Math.max(0, (($('.store_overlay').width() + 10 - $(this).outerWidth()) / 2) + $('.store_overlay').scrollLeft()) + "px");
	return this;
}
...
function openStore(open='') {
	$.ajax( {
		type: 'GET', url: '/habblet/store', success: function(data) {
			$(data).hide().appendTo('body');
			$(".store").center();
			$(".store").draggable( {
				containment: "body", cursor: 'move'
			}
			);
			if (open=='') {
...
function placeElement(selector) {
	var elem=$(selector), id=elem.attr('data-id'), name=elem.attr('data-name'), type=elem.attr('data-type');
	var value=JSON.stringify( {
		"id": id, "name": name
	}
	);
	$.ajax( {
		...
		, type: 'POST', url: '/home/' + username + '/add', contentType: 'application/json', data: value, success: function(result) {
			if (result.type=='s') {
...
function storePreview(selector) {
	...
	switch (type) {
		case "b-inv": $('.store_image').removeClass('preview_sticker');
		...
		break;
		case "s-inv": $('.store_image').addClass('preview_sticker');
		...
		$('.store_controls').html(c);
		break;
		case "s-store": $('.store_image').addClass('preview_sticker');
		$('.store_image').css('background-image', 'url(/images/homestickers/' + name + '.gif)');
		var c=$('<button onclick="buyElement(this)" data-name="' + name + '" data-type="s" data-id="' + id + '">Purchase</button>');
		$('.store_controls').html(c);
		break;
		case "b-store": $('.store_image').removeClass('preview_sticker');
		$('.store_image').css('background-image', 'url(/images/profile_backgrounds/' + name + ')');
		...
		$('.store_controls').html(c);
		break;
		case "w-inv": console.log('reached here');
...
function previewBg(selector) {
	var elem=$(selector), name=elem.attr('data-name');
...
function changeBg(selector) {
	var elem=$(selector), id=elem.attr('data-id'), name=elem.attr('data-name');
	type='b' var value=JSON.stringify( {
...
function buyElement(selector) {
	var elem=$(selector), id=elem.attr('data-id'), name=elem.attr('data-name'), type=elem.attr('data-type'), value=JSON.stringify( {
		"id": id, "name": name, "type": type
	}
	);
	$.ajax( {
		...
		, type: 'POST', url: '/home/' + username + '/buy', contentType: 'application/json', data: value, success: function(result) {
			if (result.status=='success') {
				$('.store-alert').html('Purchased!');
				$('.store-alert').fadeIn(200);
				var oldCount=parseInt($('#newItemsCount').text(), 10);
				var items=oldCount + 1;
				console.log($('#newItemsCount').text()) console.log(oldCount) console.log(items) $('#newItems').show();
				$('#newItemsCount').html(items) setTimeout(function() {
					$('.store-alert').fadeOut(200, function() {
						$(this).html();
					}
					);
				}
				, 1000) if (result.type=='s') {
...
function initNote() {
	$("#noteForm").submit(function(e) {
		...
		if(!text) {
			...
			$.ajax( {
				type: "POST", url: '/home/' + username + '/note', data: form.serialize(), success: function(result) {
...
function nl2br (str, is_xhtml) {
	...
	str=str.replace('</script>', '&#60;/script&#62;');
	str=str.replace(/\[b\]/gi, '<b>') str=str.replace(/\[\/b\]/g, '</b>') str=str.replace(/\[i\]/g, '<i>') str=str.replace(/\[\/i\]/g, '</i>') str=str.replace(/\[\/color\]/g, '</span>') str=str.replace(/\[color=(.+?)\]/g, '<span style="color:$1;">');
	return (str + '').replace(/([^>\r\n]?)(\r\n|\n\r|\r|\n)/g, '$1' + breakTag + '$2');
}
